...
Field firmware updates over CAN are standard in automotive ECUs, and a bootloader that can be interrupted at any point without bricking the device is a real engineering problem, not a demo. This project implements the full path: flash partitioning, a UDS-based update session over ISO-TP, integrity verification, and recovery from failed or interrupted updates.

It builds on my earlier [vehicle CAN bus simulator](https://github.com/GamzeAkcayy/stm32-can-node) (STM32 + Raspberry Pi, FreeRTOS, SocketCAN).
...
| Application | Updatable firmware, own vector table      | Entered only if metadata valid |
...
|------------------------------------------|-------|-----------------------------|
| Update time for 64 KB image              | TBD   | CAN baud rate, block size   |
| Recovery success after power loss        | TBD   | N interruptions at random points |
| Bootloader size                          | TBD   | Compiler, optimization flags |
| Corrupted image rejected                 | TBD   | N injected faults           |

## Roadmap

- [ ] Linker scripts for bootloader and application partitions
- [ ] Safe jump from bootloader to application
